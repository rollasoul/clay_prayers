...
# start densecap neural network to generate captions, repeat if no proper lines are created
no_find = 0
timeout = 0
ag_count = 0
while no_find == 0 and ag_count < 4:
  ag_count = ag_count + 1
  ...
  while len(check_noun) != len(set(check_noun)):
	...
	#print word_check
  	...
  	if time.time() > timeout:
      		no_find = 0
      		print "look at it again"
		break
  if no_find ==1 or ag_count > 2:
